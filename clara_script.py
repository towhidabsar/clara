...
def batch_run_json(a,b,name,problem, correct, problems, correct_path, incorrect_path):
    logging.info("Thread %s: starting", name)
    for x in range(a, b):
        # incorrect file
        ifile = problems[x]
        ...
        if (not os.path.exists(f'/data/batch_tests/{problem}/')):
            os.makedirs(f'/data/batch_tests/{problem}/')
        with open(f'/data/batch_tests/{problem}/{incorrect_file_no}_{str(g)}.json') as wb:
            json.dump(wb)
...
def main(lst):
    for problem_name in lst:
        if problem_name not in ['ProblemList.txt', 'SolutionLists']:
            ...
            for i in range(4):
                x = threading.Thread(target=batch_run_json, args=(indexes[i], indexes[i+1], i, problem_name, correct, probs, correct_path, incorrect_path))
                threads.append(x)
                x.start()
